...
class Utils {
	...
	static randomFromTo (min: number, max: number, floored: boolean = false): number {
		const random = Math.random() * (max - min + 1) + min;
		return floored ? Math.floor(random) : random;
	};

	static createArray <T>(length: number, fill: T): Array<T> {
		return Array.from({ length }, () => fill);
	};

	static create2DArray <T>(height: number, width: number, fill: T): Array<Array<T>> {
		return Array.from({ length: height }, () => Array.from({ length: width }, () => fill));
	};

	static isColiding (block: Block, field: Field, pos: Vector): boolean {
		const { x, y } = pos;
